...
# Persistent field values for Add Dialog
if "add_brand_value"       not in st.session_state: st.session_state.add_brand_value = ""
if "add_model_value"       not in st.session_state: st.session_state.add_model_value = ""
if "add_serial_value"      not in st.session_state: st.session_state.add_serial_value = ""
if "add_warranty_value"    not in st.session_state: st.session_state.add_warranty_value = ""
if "add_status_value"      not in st.session_state: st.session_state.add_status_value = ""
if "add_received_value"    not in st.session_state: st.session_state.add_received_value = ""
...
def _resolve(value_key: str, native_key: str) -> str:
    ss = st.session_state
    if value_key in ss and ss[value_key] is not None and str(ss[value_key]).strip() != "":
...
def manage_users_dialog():
    ...
    if mc2.button("❌ Close", use_container_width=True, key="admin_add_cancel"):
        reset_dialog_state()
        st.rerun()
...
#     #         placeholder="Brand",
#     #         value=st.session_state.get("add_brand_value", ""),
#     #         key="add_brand"
#     #     )
...
#             placeholder="Model",
#             value=st.session_state.get("add_model_value", ""),
#             key="add_model"
#         )
...
#             placeholder="S No.",
#             value=st.session_state.get("add_serial_value", ""),
#             key="add_serial"
#         )
...
#             value=st.session_state.get("add_warranty_value", ""),
#             key="add_warranty"
#         )
...
#             placeholder="Status",
#             value=st.session_state.get("add_status_value", ""),
#             key="add_status"
#         )
...
#             value=st.session_state.get("add_received_value", ""),
#             key="add_received"
#         )
...
#     with r1c1:
#         st.markdown(
#             "Brand <span style='color:red'>*</span>", unsafe_allow_html=True
#         )
#         brand_input = st_smart_text_input(
#             label="",
#             options=brand_options,
...
#             "Quantity",
#             min_value=1,
#             value=st.session_state.get("add_qty", 1),
#             key="add_qty",
#         )
...
#         "💾 Save", key="add_save_btn", use_container_width=True, type="primary"
#     )
#     cancel_clicked = col2.button(
#         "❌ Cancel", key="add_cancel_btn", use_container_width=True
#     )
...
#             if _issue_type == "Set Date"
#             else None
#         )
#         return_date = (
#             st.session_state.get("add_return_date", None)
#             if _return_type == "Set Date"
#             else None
#         )
...
def add_inventory_dialog():
    ...
    with r1c1:
        ...
        # Fix: Always sync state (convert None or empty to clean string/strip)
        st.session_state["add_brand_value"] = (
            brand_input.strip() if brand_input else ""
        )
    ...
    with r1c2:
        st.markdown("Category")
        # Adds vertical spacing so the input box aligns perfectly with smart text inputs
        st.markdown(
            "<div style='margin-top: 34px;'></div>", unsafe_allow_html=True
        )
        st.text_input(
            "",
            value=st.session_state.get("add_category", "Laptop"),
    ...
    with r1c3:
        ...
        # Fix: Always sync state
        st.session_state["add_model_value"] = (
            model_input.strip() if model_input else ""
        )
    ...
    with r1c4:
        ...
        # Fix: Always sync state
        st.session_state["add_serial_value"] = (
            serial_input.strip() if serial_input else ""
        )
    ...
    with r2c2:
        ...
        st.session_state["add_warranty_value"] = (
            warranty_input.strip() if warranty_input else ""
        )
    ...
    with r2c3:
        ...
        st.session_state["add_status_value"] = (
            status_input.strip() if status_input else ""
        )
    ...
    with r3c2:
        ...
        st.session_state["add_received_value"] = (
            received_input.strip() if received_input else ""
        )
    ...
    if save_clicked:
        # Pull directly from session state or widget keys
        final_brand = st.session_state.get(
            "add_brand_value", ""
        ) or st.session_state.get("add_brand", "")
        final_model = st.session_state.get(
            "add_model_value", ""
        ) or st.session_state.get("add_model", "")
        final_serial = st.session_state.get(
            "add_serial_value", ""
        ) or st.session_state.get("add_serial", "")
        final_warranty = st.session_state.get(
            "add_warranty_value", ""
        ) or st.session_state.get("add_warranty", "")
        final_status = st.session_state.get(
            "add_status_value", ""
        ) or st.session_state.get("add_status", "")
        final_received = st.session_state.get(
            "add_received_value", ""
        ) or st.session_state.get("add_received", "")
...
# ═════════════════════════════════════════════════
# EDIT INVENTORY DIALOG (STANDARD INPUTS)
# ═════════════════════════════════════════════════
                ...
                with c_resend:
                    if st.button("Resend email", use_container_width=True):
                        if st.session_state.verification_code and st.session_state.pending_user_data:
                            ...
                            st.success("✅ Code re-transmitted successfully.")
                with c_back:
