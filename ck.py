...
def view_profile_dialog():
    ...
    # st.markdown(f"**Current Password:** `{type(user_info['password'])}`")
    st.divider()
...
# ═══════════════════════════════
@st.dialog("⚙️ User Account Management", width="large")
def manage_users_dialog():
    st.subheader("Existing Accounts Overview")
    df_users = fetch_all_users()
    st.dataframe(df_users, use_container_width=True, hide_index=True)

    st.divider()
    st.subheader("➕ Create New Account (Admin Controlled)")
...
#     with r1c2:
#         st.markdown("Category")
#         category = st.text_input("", value=row["item_category"] or "", key="edit_category", label_visibility="collapsed")
#     with r1c3:
#         st.markdown("Model <span style='color:red'>*</span>", unsafe_allow_html=True)
#         model_input = st_smart_text_input(
#             label="",
...
#         qty = st.number_input(
#             "Quantity",
#             value=int(row["quantity"]) if (row["quantity"] is not None and str(row["quantity"]).isdigit()) else 1,
#             min_value=1,
...
#             key="edit_received",
#             value=row.get("received_from") or ""
#         )
#         if received_input is not None:
...
#     # PRE-LOAD SESSION STATE DEFAULTS IF NOT ALREADY SET
...
#             options=brand_options,
#             placeholder="Brand",
#             key="edit_brand"  # Loaded from session state
#         )
#         if brand_input is not None:
#             st.session_state["edit_brand_value"] = brand_input
#         if "brand" in errors:
...
#             options=model_options,
#             placeholder="Model",
#             key="edit_model"  # Loaded from session state
#         )
#         if model_input is not None:
#             st.session_state["edit_model_value"] = model_input
#         if "model" in errors:
...
#             options=serial_options,
#             placeholder="Serial No.",
#             key="edit_serial"  # Loaded from session state
#         )
#         if serial_input is not None:
#             st.session_state["edit_serial_value"] = serial_input
#         if "serial" in errors:
...
#             options=warranty_options,
#             placeholder="Warranty Status",
#             key="edit_warranty"  # Loaded from session state
#         )
#         if warranty_input is not None:
#             st.session_state["edit_warranty_value"] = warranty_input
...
#             options=status_options,
#             placeholder="Status",
#             key="edit_status"  # Loaded from session state
#         )
#         if status_input is not None:
#             st.session_state["edit_status_value"] = status_input
...
#             options=received_options,
#             placeholder="Received From",
#             key="edit_received"  # Loaded from session state
#         )
#         if received_input is not None:
#             st.session_state["edit_received_value"] = received_input
    ...
    with center_col:
        ...
        with st.container(border=True):
            ...
            # ─── MODE: VERIFY ───
                ...
                st.markdown(f"<h4 style='text-align: center; color: #555;'>Check your inbox</h4>", unsafe_allow_html=True)
...
# with profile_col:
#     if st.button("", icon=":material/account_circle:", key="profile_btn", use_container_width=True, help="View / Edit Profile"):
