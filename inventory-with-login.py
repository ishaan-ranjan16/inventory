...
def hash_password(password: str) -> str:
    """Securely hash a cleartext password with bcrypt."""
    salt = bcrypt.gensalt(rounds=12)
    return bcrypt.hashpw(password.encode("utf-8"), salt).decode("utf-8")

def verify_password(password: str, hashed: str) -> bool:
    """Verify password attempts against secure database bcrypt hashes."""
    try:
...
def register_user(username, password):
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("INSERT INTO users (username, password) VALUES (?, ?)", (username_clean, hashed_pass))
        conn.commit()
        cur.close()
        conn.close()
...
def reset_user_password(username, new_password):
    """Overwrites user hash strictly upon verification/forgot password execution."""
    username_clean = username.strip()
    if not user_exists(username_clean):
...
#         st.subheader("🔑 Operations Portal Log In")
#         login_user = st.text_input("Username", key="login_username_field").strip()
#         login_pass = st.text_input("Password", type="password", key="login_password_field")
...
#         st.subheader("📝 Register Operational Account")
...
#         if st.button("Register & Save Hash", type="primary", use_container_width=True):
...
#         st.caption("For security, resetting will immediately hash and replace your credential payload.")
#         reset_user = st.text_input("Enter Registered Username", key="reset_username").strip()
#         reset_pass = st.text_input("Enter New Password", type="password", key="reset_pass")
...
#                 st.error("Password must be at least 6 characters long.")
#             else:
#                 success, msg = reset_user_password(reset_user, reset_pass)
#                 if success:
#                     st.success(msg)
#                     time.sleep(1)
#                     st.session_state.auth_mode = "login"
...
# #  NEW NATIVE STREAMLIT CONTAINER FIX:
# if not st.session_state.logged_in:
    
#     # We use st.columns to center our native container card on the screen
#     _, center_col, _ = st.columns([1, 1.5, 1])
    
#     with center_col:
#         # st.container(border=True) creates a clean, secure card wrapper
...
if not st.session_state.logged_in:
    ...
    # Centers the login card layout on the screen
    _, center_col, _ = st.columns([1, 2, 1])
    
    with center_col:
        # --- COMPANY LOGO CONTAINER ---
        # A clean, bounded border box for your rectangular brand image
        with st.container(border=True):
            # Replace "path_to_logo.png" with your actual local file path or image URL
            # use_container_width ensures it behaves correctly inside the box
            st.image(
                "vrizelogo.png", 
                caption="Inventory Portal", 
        ...
        with st.container(border=True):
            if st.session_state.auth_mode == "login":
                st.subheader("🔑 Inventory Portal - Login")
                login_user = st.text_input("Username", key="login_username_field").strip()
                login_pass = st.text_input("Password", type="password", key="login_password_field")
