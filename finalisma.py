...
if "user_search_query" not in st.session_state: st.session_state.user_search_query = ""
...
def manage_users_dialog():
    ...
    sc1, sc2 = st.columns([0.8, 0.2], vertical_alignment="bottom", gap="small")
    with sc1:
        st.text_input(
            "Search Users",
    ...
    if st.session_state.user_search_query.strip():
        q = st.session_state.user_search_query.strip().lower()
        df_users = df_users[
    ...
    U_COL_WIDTHS = [2.2, 3.5, 1.8, 0.8, 0.8]
    
    with st.container(key="user_header_row"):
        uh0, uh1, uh2, uh3, uh4 = st.columns(U_COL_WIDTHS, gap="small")
        uh0.badge("Username")
        uh1.badge("Email")
        uh2.badge("Role")
        uh3.badge("Edit")
    ...
    if not df_users.empty:
        for idx, urow in df_users.iterrows():
            ...
            with st.container(key=f"u_row_{idx}"):
                uc0, uc1, uc2, uc3, uc4 = st.columns(U_COL_WIDTHS, gap="small")
                
                uc0.markdown(sm_txt(uname_val), unsafe_allow_html=True)
                uc1.markdown(sm_txt(email_val), unsafe_allow_html=True)
                uc2.markdown(sm_txt(role_val),  unsafe_allow_html=True)
                
                if uc3.button("✏️", key=f"u_edit_{idx}"):
                    st.session_state.edit_user_data = {"username": uname_val, "email": email_val, "role": role_val}
                    st.rerun()
                    
                if uc4.button(":material/delete:", key=f"u_del_{idx}"):
...
#     with r1c2:
#         st.markdown("Category")
#         st.write("")   # Adds one line of vertical space
#         # st.markdown("<div style='height:8px;'></div>", unsafe_allow_html=True)
#         st.text_input(
#             "",
#             value=st.session_state.get("add_category", "Laptop"),
#             key="add_category",
#             label_visibility="collapsed",
#         )
...
def add_inventory_dialog():
    ...
    #         st.session_state["add_category"] = "Laptop"
    ...
    with r1c2:
        st.markdown("Category")
        # Custom vertical spacer to match the height of smart text inputs
    ...
    with r2c1:
        ...
        st.number_input("Quantity", min_value=1, key="add_qty")
...
# #     with r1c1:
# #         st.markdown("Brand <span style='color:red'>*</span>", unsafe_allow_html=True)
# #     # Initialize session state first if it doesn't exist yet
# #         if "edit_brand" not in st.session_state:
# #             st.session_state["edit_brand"] = row.get("brand") or ""
            
# #         st.text_input(
# #             "",
# #             key="edit_brand",  # Streamlit will automatically use st.session_state["edit_brand"]
# #             label_visibility="collapsed"
# #             )
...
# #     #     st.text_input(
# #     #         "",
# #     #         value=st.session_state.get("edit_brand", row.get("brand") or ""),
# #     #         key="edit_brand",
# #     #         label_visibility="collapsed"
# #     #     )
# #     #     if "brand" in errors:
...
# #     #     st.text_input(
# #     #         "",
# #     #         value=st.session_state.get("edit_category", row.get("item_category") or "Laptop"),
# #     #         key="edit_category",
# #     #         label_visibility="collapsed"
# #     #     )
# #     with r1c2:
# #         st.markdown("Category")
# #         # Ensure session state is initialized before rendering the widget
# #         if "edit_category" not in st.session_state:
# #             st.session_state["edit_category"] = row.get("item_category") or "Laptop"
...
# #         st.text_input(
# #             "",
# #             value=st.session_state.get("edit_model", row.get("model") or ""),
# #             key="edit_model",
# #             label_visibility="collapsed"
# #         )
# #         if "model" in errors:
...
# #         st.text_input(
# #             "",
# #             value=st.session_state.get("edit_serial", row.get("serial_no") or ""),
# #             key="edit_serial",
# #             label_visibility="collapsed"
# #         )
# #         if "serial" in errors:
...
# #             "Quantity",
# #             min_value=1,
# #             value=int(st.session_state.get("edit_qty", row.get("quantity") or 1)),
# #             key="edit_qty"
# #         )
...
# #         st.text_input(
# #             "Warranty Status",
# #             value=st.session_state.get("edit_warranty", row.get("warranty_status") or ""),
# #             key="edit_warranty"
# #         )
...
# #         st.text_input(
# #             "Status",
# #             value=st.session_state.get("edit_status", row.get("status") or ""),
# #             key="edit_status"
# #         )
...
# #         st.text_input(
# #             "Status-2",
# #             value=st.session_state.get("edit_status2", row.get("status_2") or ""),
# #             key="edit_status2"
# #         )
...
# #         st.text_input(
# #             "Handover To",
# #             value=st.session_state.get("edit_handover", row.get("hand_over_to") or ""),
# #             key="edit_handover"
# #         )
...
# #         st.text_input(
# #             "Received From",
# #             value=st.session_state.get("edit_received", row.get("received_from") or ""),
# #             key="edit_received"
# #         )
...
# #     r4c1, r4c2 = st.columns(2)
# #     with r4c1:
# #         default_issue_index = 0 if row.get("issue_date") else 1
# #         issue_type = st.radio(
# #             "📅 Issue Date",
# #             ["Set Date", "NA (not issued/in-inventory/available)"],
# #             horizontal=True, index=default_issue_index, key="edit_issue_type",
# #         )
# #         if issue_type == "Set Date":
# #             st.date_input(
# #                 "Issue Date", value=safe_date(row.get("issue_date")),
# #                 key="edit_issue_date", label_visibility="collapsed",
# #             )

# #     with r4c2:
# #         default_return_index = 0 if row.get("return_date") else 1
# #         return_type = st.radio(
# #             "📅 Return Date",
# #             ["Set Date", "NA (in-inventory/not returned/in-use)"],
# #             horizontal=True, index=default_return_index, key="edit_return_type",
# #         )
# #         if return_type == "Set Date":
# #             st.date_input(
# #                 "Return Date", value=safe_date(row.get("return_date")),
# #                 key="edit_return_date", label_visibility="collapsed",
# #             )

# #     st.markdown("<hr style='margin:6px 0;'>", unsafe_allow_html=True)
# #     st.text_area(
# #         "📝 Note",
# #         value=st.session_state.get("edit_note", row.get("note") or ""),
# #         key="edit_note",
# #         height=80
# #     )
...
# #     # ── Buttons ──
# #     col1, col2 = st.columns(2)
# #     save_clicked   = col1.button("💾 Save",   key="edit_save_btn",   use_container_width=True, type="primary")
# #     cancel_clicked = col2.button("❌ Cancel", key="edit_cancel_btn", use_container_width=True)
...
# #         issue_date  = st.session_state.get("edit_issue_date",  None) if _issue_type  == "Set Date" else None
# #         return_date = st.session_state.get("edit_return_date", None) if _return_type == "Set Date" else None

# #         update_inventory((
# #             final_brand,
# #             final_model,
# #             final_serial,
# #             st.session_state.get("edit_category", "") or "",
# #             final_warranty,
# #             st.session_state.get("edit_qty", 1),
# #             final_status,
# #             st.session_state.get("edit_handover", "") or "",
# #             issue_date,
# #             final_received,
# #             return_date,
# #             st.session_state.get("edit_note", "") or "",
# #             st.session_state.get("edit_status2", "") or "",
# #             row["id"]
# #         ))
...
def edit_inventory_dialog():
    ...
    with r2c1:
        if "edit_qty" not in st.session_state:
            st.session_state["edit_qty"] = int(row.get("quantity") or 1)
    ...
    with center_col:
        ...
        with st.container(border=True):
            ...
            # ─── MODE: FORGOT PASSWORD ───
                ...
                f_user = st.text_input("Username").strip()
                f_email = st.text_input("Email").strip()
                f_pass = st.text_input("New Password", type="password")
                
                if st.button("Update Password", type="primary", use_container_width=True):
                    if not f_user or not f_email or not f_pass:
                        st.error("⚠️ All fields are required.")
                    else:
                        succ, msg = reset_user_password(f_user, f_email, f_pass)
                        if succ:
                            st.success(msg)
                            time.sleep(1)
                            st.session_state.auth_mode = "login"
...
# ══════════════════════════════════════════
# MAIN APP PORTAL (LOGGED IN USER DASHBOARD)
# ══════════════════════════════════════════
...
# TOP HEADER ROW
# ══════════════
title_col, add_col, manage_col, pdf_col, excel_col, logout_col, prof_col = st.columns(
    [4.2, 0.9, 0.96, 0.9, 0.9, 0.63, 0.3], vertical_alignment="center"
)

with title_col:
    role_badge = "👑 Admin" if st.session_state.user_role == "ADMIN" else "👤 User"
    # st.badge(f"**:material/inventory: Overall Inventory Status** (`{st.session_state.username}` - {role_badge})", color="green")
    st.badge(f"**:material/inventory: Overall Inventory Status**", color="green")
...
with prof_col:
    if st.button("", icon=":material/account_circle:", key="profile_btn", use_container_width=True):
...
with logout_col:
    if st.button("Logout", icon=":material/logout:", key="logout_btn", use_container_width=True):
...
for _, row in list_df.iterrows():
    ...
    with st.container(key=f"row_{uid}"):
        ...
        if c14.button("✏️", key=f"edit_{uid}"):
            ...
            # Pre-seed edit field values into session state for text input fields
            st.session_state["edit_brand"]    = row.get("brand") or ""
            st.session_state["edit_model"]    = row.get("model") or ""
            st.session_state["edit_serial"]   = row.get("serial_no") or ""
