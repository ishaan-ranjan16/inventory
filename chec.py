...
# Import our unified connection function
...
def register_user(username, email, password):
    ...
    try:
        ...
        cur.execute("INSERT INTO users (username, email, password) VALUES (?, ?, ?)", 
                    (username_clean, email_clean, hashed_pass))
        conn.commit()
        cur.close()
        conn.close()
...
def verify_user_login(username, password):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT password FROM users WHERE username = ?", (username_clean,))
    row = cur.fetchone()
    cur.close()
    conn.close()
...
def reset_user_password(username, email, new_password):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT email FROM users WHERE username = ?", (username_clean,))
    row = cur.fetchone()
    ...
    try:
        cur.execute("UPDATE users SET password = ? WHERE username = ?", (new_hashed, username_clean))
        conn.commit()
        cur.close()
        conn.close()
...
def send_gmail_notification(subject: str, body_content: str):
    try:
        ...
        st.warning("⚠️ Background alert couldn't be sent: Missing SMTP keys in secured file.")
        return
...
def add_inventory_dialog():
    ...
    if save_clicked:
        ...
        # Display success inside dialog box context explicitly
        st.success("#### ✅ Saved Successfully!")
        
        # Reset specific operational key state before rerunning app layout out of dialog overlay
        st.session_state.add_errors    = {}
        st.session_state.active_dialog = None
        for k in [k for k in st.session_state if k.startswith("add_")]:
            st.session_state.pop(k, None)
...
def edit_inventory_dialog():
    ...
    if save:
        ...
        # Output layout confirmation status inside open context dialog element prior to teardown
...
def confirm_delete_dialog():
    ...
    with c1:
        if st.button("🗑️ Yes, Delete", type="primary", use_container_width=True):
            ...
            # Displays message inside context layer box cleanly
...
if not st.session_state.logged_in:
    ...
    with center_col:
        ...
        with st.container(border=True):
            if st.session_state.auth_mode == "login":
                st.markdown("<h3 style='text-align: center; margin-bottom: 20px;'>🔐Inventory - Login</h3>", unsafe_allow_html=True)
                ...
                if st.button("Login", type="primary", use_container_width=True):
                    if not login_user or not login_pass:
                    ...
                    elif verify_user_login(login_user, login_pass):
                        ...
                        st.error("⚠️ Invalid credentials entered.")
                ...
                st.markdown("<h3 style='text-align: center; margin-bottom: 20px;'>📝 Register / Create Account</h3>", unsafe_allow_html=True)
                reg_user = st.text_input("**:material/person: Enter Username**", key="reg_username").strip()
                reg_email = st.text_input("**:material/email: Enter Email Address**", placeholder="name@company.com", key="reg_email").strip()
                reg_pass = st.text_input("**:material/lock: Enter Password**", type="password", key="reg_password")
                reg_confirm = st.text_input("**:material/lock: Confirm Password**", type="password", key="reg_confirm")
                
                if st.button("Register & Save", type="primary", use_container_width=True):
                    if not reg_user or not reg_email or not reg_pass:
                        st.toast("⚠️ Username, Email, and Password are all required.")
    ...
    st.markdown(
        ...
        <div style="text-align:center; font-size:12px; color:gray;">
            © 2026 Vrize. All rights reserved.
        </div>
        """,
        unsafe_allow_html=True
    )
...
title_col, add_col, pdf_col, excel_col, logout_col = st.columns(
    [5, 1.1, 1.1, 1.1, 0.8], vertical_alignment="center"
)

with title_col:
    st.badge("**:material/inventory: Overall Inventory Status**", color="green")
    st.badge(f":material/person: Active User: **{st.session_state.username}**", color="violet")
