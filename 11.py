...
def manage_users_dialog():
    # Helper to helper-render small text entries
    def sm_txt(v):
    ...
    if st.session_state.edit_user_data:
        ...
        "\n [Leave blank to keep current password]", type="password", help="", key="m_edit_pass")
        ...
        if bc1.button("💾 Save Changes", type="primary", use_container_width=True):
            if not e_uname.strip() or not e_email.strip():
                ...
                if succ:
                    st.success(msg)
                    # If edited self username, sync active session
    ...
    st.badge("**:material/inventory: Existing Accounts Overview -**",color="green")

    df_users = fetch_all_users()

    # Search + Refresh controls aligned right relative to content block
...
def edit_inventory_dialog():
    ...
    # Fetch distinct options for autocomplete fields
    brand_options    = fetch_distinct("brand")
    model_options    = fetch_distinct("model")
    serial_options   = fetch_distinct("serial_no")
    warranty_options = fetch_distinct("warranty_status")
    ...
    # Seed Session State keys with record defaults if they are not already set
    ...
    with r1c1:
        ...
        if brand_input is not None:
            st.session_state["edit_brand_value"] = brand_input
    ...
    with r1c2:
        ...
        category = st.text_input(
            "",
            value=row.get("item_category") or "",
            key="edit_category",
            label_visibility="collapsed"
        )
    ...
    with r1c3:
        ...
        if model_input is not None:
            st.session_state["edit_model_value"] = model_input
    ...
    with r1c4:
        ...
        if serial_input is not None:
            st.session_state["edit_serial_value"] = serial_input
    ...
    with r2c1:
        qty = st.number_input(
            "Quantity",
            value=int(row["quantity"]) if (row.get("quantity") is not None and str(row["quantity"]).isdigit()) else 1,
            min_value=1,
            key=f"edit_qty_{row['id']}"
        )
    ...
    with r2c2:
        ...
        if warranty_input is not None:
            st.session_state["edit_warranty_value"] = warranty_input
    ...
    with r2c3:
        ...
        if status_input is not None:
            st.session_state["edit_status_value"] = status_input

    with r2c4:
        status_2 = st.text_input("Status-2", value=row.get("status_2") or "", key=f"edit_status2_{row['id']}")
    ...
    with r3c1:
        handover = st.text_input("Handover To", value=row.get("hand_over_to") or "", key=f"edit_handover_{row['id']}")
    ...
    with r3c2:
        ...
        if received_input is not None:
            st.session_state["edit_received_value"] = received_input
    ...
    with r4c1:
        issue_type = st.radio(
            ...
            ["Set Date", "NA (not issued/in-inventory/available)"],
            horizontal=True,
            index=0 if row.get("issue_date") else 1,
            key=f"issue_{row['id']}",
        )
    ...
    with r4c2:
        return_type = st.radio(
            ...
            ["Set Date", "NA (in-inventory/not returned/in-use)"],
            horizontal=True,
            index=0 if row.get("return_date") else 1,
    ...
    st.markdown("<hr style='margin:6px 0;'>", unsafe_allow_html=True)
    note = st.text_area("📝 Note", value=row.get("note") or "", height=80, key=f"edit_note_{row['id']}")

    c1, c2 = st.columns(2)
    save   = c1.button("💾 Save",   use_container_width=True, type="primary")
    cancel = c2.button("❌ Cancel", use_container_width=True)

    if save:
        final_brand    = _resolve("edit_brand_value",    "edit_brand")
        final_model    = _resolve("edit_model_value",    "edit_model")
        final_serial   = _resolve("edit_serial_value",   "edit_serial")
        final_warranty = _resolve("edit_warranty_value", "edit_warranty")
        final_status   = _resolve("edit_status_value",   "edit_status")
        final_received = _resolve("edit_received_value", "edit_received")
...
if not st.session_state.logged_in:
    ...
    with center_col:
        ...
        with st.container(border=True):
            ...
            #     st.caption("Verify your username and matching registered email to create a new password.")
            #     reset_user = st.text_input("**:material/person: Enter Registered Username**", key="reset_username").strip()
            #     reset_email = st.text_input("**:material/email: Enter Registered Email**", key="reset_email").strip()
            ...
            #             st.toast("⚠️ All fields are required.")
            #         elif reset_pass != reset_confirm:
            #             st.toast("⚠️ New passwords do not match!")
            #         elif len(reset_pass) < 6:
            #             st.toast("⚠️ Password must be at least 6 characters long.")
            #         else:
            #             success, msg = reset_user_password(reset_user, reset_email, reset_pass)
            #             if success:
            #                 st.success(msg)
            #                 time.sleep(1)
            #                 st.session_state.auth_mode = "login"
            ...
            # ─── MODE: FORGOT ───
                ...
                # STEP 1: Enter Username and Email to request OTP
                    ...
                    if st.button("Update Password", type="primary", use_container_width=True):
                        if not reset_pass or not reset_confirm:
                            ...
                            if success:
                                ...
                                # Clear temporary reset session keys
                ...
                st.markdown("---")
                if st.button("← Back to Login", use_container_width=True):
                    # Reset temporary state variables when canceling
                    for key in ["forgot_otp_sent", "forgot_otp_verified", "reset_target_user", "reset_target_email", "reset_verification_code"]:
                        st.session_state.pop(key, None)
                    st.session_state.auth_mode = "login"
                    st.rerun()
...
if st.session_state.user_role == "ADMIN":
    title_col, add_col, manage_col, pdf_col, excel_col, logout_col, profile_col = st.columns(
        [4, 1.1, 1.2, 1.1, 1.1, 0.8, 0.6], vertical_alignment="center"
    )
else:
    title_col, add_col, pdf_col, excel_col, logout_col, profile_col = st.columns(
        [5, 1.1, 1.1, 1.1, 0.8, 0.6], vertical_alignment="center"
    )
